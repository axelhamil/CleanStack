...
Something broke? The flow is always the same:

```
Controller → Use Case → Domain → Repository
```

You know exactly where to look. You don't need to understand the whole system. You just need to follow the layers.
...
if (result.isFailure) {
  // You MUST handle this. The compiler ensures it.
...
The mental model is wrong. People imagine Clean Architecture as *extra work on top of* writing code. Layers to maintain, abstractions to juggle, more files to create. It sounds exhausting.

But that's not how it works in practice. **You're writing the same amount of code.** You're just putting it in different places.

Think about it: whether you use Clean Architecture or not, you still need to:
- Validate user input
...
The difference is *where* that code lives. In a vibe-coded app, it's all mixed together in one file - validation next to database queries next to error handling. In Clean Architecture, each concern has a home.

**Same code. Same effort. Different organization.**

The "extra" files aren't busywork - they're boundaries. And boundaries are what make code navigable. When everything is in one place, you have to understand everything to change anything. When concerns are separated, you only touch what you need.

Here's the irony: Clean Architecture feels like more work *at the start*, but it's actually less work *over time*. The vibe-coded app is fast to write and slow to maintain. The structured app is the same speed to write and fast to maintain.

And with AI? The difference is even starker. AI generates Clean Architecture code just as fast as spaghetti code. But when that AI-generated code needs debugging, one is archaeology and the other is a clear path.

**This isn't enterprise overhead for your side project. It's the same effort with 10x better outcomes.**
...
- [Core Concepts](/docs/core-concepts) - DDD patterns explained
- [Architecture](/docs/architecture) - Clean Architecture layers

**Guides**
- [Authentication](/docs/guides/authentication) - Complete auth example (100% Claude Code)
- [First Use Case](/docs/guides/first-use-case) - Build your first feature
- [Transactions](/docs/guides/transactions) - Multi-step operations
- [Testing](/docs/guides/testing) - BDD test strategy

**AI Development**
- [AI Overview](/docs/ai) - Why CleanStack + AI works
- [Claude Code](/docs/ai/claude-code) - Recommended AI tool
- [Prompts](/docs/ai/prompts) - Ready-to-use prompts

## Tech Stack

Next.js 16 • TypeScript 5.9 • Drizzle ORM • PostgreSQL • BetterAuth • Turborepo • Tailwind CSS 4 • shadcn/ui • Biome • Vitest
